...
The formula environment generated by `model_formula()` is now a minimal two-binding environment (`poly`, `s`) with `baseenv()` as parent, rather than the full caller frame. This prevents large objects (e.g. the `df` dataframe and intermediate variables) from being serialised when a fitted model is saved with `saveRDS()`.
