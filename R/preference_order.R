...
#' @param f (optional: function) Function to compute the association between `response` and each predictor in `predictors`. If NULL (default), the function returned by [f_default()] is used:
#' \itemize{
...
#' #categoricals transformed to numeric via target encoding
...
#'
#' #numeric response and mixed predictors
#' #disabled target encoding, categoricals used as is
...
preference_order <- function(
  ...
  attr(x = preference, which = "f_name") <- f_name
