...
#'
#' @description
#' Fits a Gaussian GLM model \code{y ~ x} with the numeric response \code{y} and the numeric, character, or factor predictor \code{x} using \code{stats::glm()} and returns the R-squared of the observations against the predictions (see [score_r2()]).
#'
...
#' }
#' @param ... (optional) Accepts the arguments \code{cv_training_fraction} (numeric between 0 and 1) and \code{cv_iterations} (integer between 1 and Inf) for cross validation.
