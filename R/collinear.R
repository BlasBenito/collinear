...
#'   \item output: one object of class `collinear_output` generated by [build.collinear_output()] per response in argument `response`.
#' }
#'
#' @examples
...
collinear <- function(
  ...
  for(response in responses){
    ...
    ## response list ----
    out.response <- build.collinear_output(
      df = df.response,
      response = response,
      predictors = predictors.response,
      preference_order = preference_order.response,
