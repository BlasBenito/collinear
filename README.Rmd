...
These methods are integrated in two main functions:
 ...
 - [collinear_auto()]: Automatically configures all parameters based on data characteristics for a simplified experience.
...
The function [collinear_auto()] is designed to reduce multicollinearity in complex modelling data with a minimal setup. In this case, it performs the following actions:
...
The functions `collinear_auto()` and `collinear()` return an object of class `collinear_output`, that has its own `print()` method.

```{r}
x
```

The object `selection` contains predictors chosen by `collinear_auto()`. We can use [vif_df()] to check their level of multicollinearity.
...
```
All VIF scores are below 2.5, meaning that `collinear_auto()` did a good job here.

### Model Fitting

The output of `collinear_auto()` can be used right away to fit exploratory models, as shown below.
