...
The R package `collinear` combines four different methods to offer a comprehensive tool for multicollinearity management in data frames for statistical analysis and machine learning modeling with numeric and/or categorical predictors:

  + 1.- Bivariate correlation (Pearson, Spearman, and Cramer's V).
  + 2.- Variance inflation factor analysis (VIF).
  + 3.- Target encoding of categorical predictors to handle them as numerics during the multicollinearity analysis.
  + 4.- A flexible feature prioritisation method to ensure that the variable selection fulfills the user's needs while compliying with robust multicollinearity criteria.
  
These methods are integrated in the flagship function of the package:
