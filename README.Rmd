...
```{r make_vi_smaller}
set.seed(1)
vi <- dplyr::slice_sample(vi, n = 1000)
```
...
The function `collinear()` is designed as a complete tool for multicollinearity management. As such, it provides access to the key functionalities offered by this package. The table below shows these functionalities, the functions implementing them, their requirements, and how to disable them within `collinear()`.
...
The code below shows a full call to `collinear()` with a numeric response and mixed predictor types.
