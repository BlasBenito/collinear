...
The plot below shows the hierarchical clustering of the correlation matrix between predictors in `vi`. Clustering groups are defined at a correlation higher than 0.5.

```{r cluster_plot, fig.width=12, fig.height=5, warning = FALSE}
...
The function `collinear()` implements four functionalities that are enabled/disabled as follows

| **Functionality** | **Enabled** | **Disabled** |
|-------------------|-------------|--------------|
| target-encoding   | numeric `response` and categorical `predictors` | `response = NULL` or `encoding_method = NULL`|
| preference order | numeric orcategorical `response` | `response = NULL` or `preference_order = NULL`|
