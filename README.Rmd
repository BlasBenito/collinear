...
The `collinear` package provides four methods for managing multicollinearity in data frames for statistical and machine learning models:

- **Target Encoding**: Converts categorical variables to numeric (requires a numeric response).
- **Preference Order**: Ranks predictors by their association with the response variable.
- **Pairwise Correlation Filtering**: Automated filtering based on pairwise correlations, suitable for both numeric and categorical predictors.
- **Variance Inflation Factor Filtering**: Filters numeric predictors based on Variance Inflation Factors.

The main function is `collinear()`, which integrates these methods. Key functions include:
...
3. **Refined `preference_order()`**: New preference functions, and automated function selection depending on data features.
4. **Simplified Target Encoding**: Parallelized for better efficiency, and new default is "loo" (leave-one-out).
...
The data frame `vi` consists of 30,000 rows and 67 columns, featuring various response types and a mix of numeric and categorical predictors.

**Responses**
Column names of response variables:

- `vi_numeric`: Numeric values of a vegetation index (all other responses are derived from this).
- `vi_counts`: Integer counts.
- `vi_binomial`: Binomial variable (0s and 1s).
- `vi_categorical`: Character variable with 5 categories ("very_low" to "very_high").
- `vi_factor`: Factor version of `vi_categorical`.

**Predictors:**
Character vectors with predictor names.
...
The function `collinear()` contains the full logic for multicollinearity management in most use cases. The behaviour of this function is controlled by these arguments:
...
The dependencies between arguments might seem rather complex at first, but they can be summarized in a couple of principles and use-case scenarios.
...
```
  ...
  + Categorical predictors are target-encoded as numeric when `response` is numeric and `encoding_method` is not NULL.
  + Pairwise correlation filtering results on categorical predictors change if target-encoding is applied

Scenarios:

  + "I want to keep my categorical predictors categorical" and/or "I dislike that target encoding non-sense": set `encoding_method` to `NULL`.
  + "I know what predictors I'd like to preserve": add them as a character vector in `preference_order`.
  + "I don't know what predictors are important": provide `response` and set `preference_order` to NULL, and `collinear()` will figure it out.

  + No response available: 
  + no response, no preference_order, categorical predictors: predictors ranked from lower to higher Cramer's V with others and filtered with pairwise correlation.
