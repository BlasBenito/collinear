...
The predictor names are returned in the same order as the preference order. If `response` is provided, `collinear()` returns non-collinear variables ordered by their association with `response`. Otherwise, the predictors are ordered from lower to higher multicollinearity.
...
This function requires a numeric `response` to transform categorical `predictors` to numeric. This transformation allows applying the same multicollinearity filtering methods to all predictors. 

In `collinear()`, this functionality is controlled by the argument `encoding_method`, which defines the encoding method, or disables the functionality entirely when `NULL`.
...
The function output shows "koppen_zone" encoded as numeric, and ready for a multicollinearity analysis along with other numeric predictors.

```{r, echo = FALSE}
kableExtra::kbl(df_toy) |> 
  kableExtra::kable_styling()
```
In the example call to `collinear()` shown above, target encoding is executed for "vi_numeric", resulting in zero categorical predictors.
...
```
On the other hand, target encoding is ignored for the categorical `response` "vi_categorical", resulting in 12 categorical predictors to be handled by the multicollinearity analysis.
...
The column "predictor" shown above aligns in order with the output of `collinear()` for the response "vi_numeric".

```{r}
selection$vi_numeric
```

If the choice made by `f_auto()` to compute associations between `response` and `predictors` is deemed inappropriate, there are many other options available, or the user can create a custom one tailored to the given data.

For example, if the relationship between `response` and `predictors` is expected to be non-linear, a univariate random forest model could be used to compute preference order.
...
```
Changes in the `f` function may indeed change the preference order, and result in a completely different predictor selection returned by `collinear()`.
