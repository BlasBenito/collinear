...
When a numeric `response` is provided, this function transforms any categorical `predictors` to numeric.

For example, the categorical predictor "koppen_zone" has character values.

```{r}
head(unique(df$koppen_zone))
```
The code chunk below creates a little toy data frame to show a clear example of this behavior.
...
When introducing this predictor in `target_encoding_lab()` with the method "loo" (leave-one-out), the data frame is grouped by the levels of the predictor, and each value in a group is re-mapped as the average of the `response` across all other cases of the same group.
...
kableExtra::kbl(df_toy) |> 
  kableExtra::kable_styling()
```
This transformation method allows applying a consistent multicollinearity filtering to both categorical and numeric predictors.

When this method is disabled or the response is not numeric, `cor_select()` becomes much slower, and `vif_select()` ignores all categorical predictors.
