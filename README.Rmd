...
The R package `collinear` combines four methods to facilitate multicollinearity management:

  + **Target Encoding**: General method to transform categorical variables to numeric. Only relevant when a numeric response is available, and there are categorical predictors in the target data frame.
  + **Preference Order**: Method to rank predictors by their univariate association with a response variable of any type. The resulting ranking is used to preserve important variables during multicollinearity filtering.
  ...
  + **Variance Inflation Factor Filtering**: Automated multicollinearity filtering based on Variance Inflation Factors. It can only handle numeric predictors, unless target encoding has been applied to the target dataset.

All these methods are integrated in the function [`collinear()`](#collinear()), which is designed as an all-in-one tool for multicollinearity filtering. Additionally, the package offers other functions that may be useful to build custom workflows:
...
The package contains a example data frame named `vi`, with 30.000 rows and 67 columns with response variables of different types and a mixture of numeric and categorical predictors.

The responses are:

  + `vi_numeric`: numeric continuous variable, values of a vegetation index (all other responses are derived from this one).
...
This function performs all steps required for and easy multicollinearity management. It has the following arguments:

  + **Input data**:
      + `df` (data frame, required): Data frame with numeric and/or categorical predictors, and optionally a response variable. 
      + `response` (character, optional): Name of a categorical, numeric, binomial, or integer response variable. The value of this argument defines the function behavior:
        + NULL (default): target encoding and preference order are disabled.
        + Otherwise:
          + If `preference_order = NULL`: preference order is computed automatically.
          + If numeric and `encoding_method` is not NULL: target encoding transforms categorical predictors to numeric.
      + `predictors` (character vector, optional): Names of the target predictors. If NULL, all column names in `df` but `response` are used instead.
   + **Target encoding**:
      + `encoding_method` (character, optional): Name of the target encoding method. If NULL, or `response` is not numeric, target encoding is disabled.
   + **Preference order**:
      + `preference_order` (character vector or data frame, optional): Ranking of predictors to help preserve important ones during multicollinearity filtering. Ignored if `response` is not provided. Valid values are:
          + NULL (default): if `response` is provided, then preference order is computed automatically.
          + Character vector: predictor names in a custom preference order.
          + Data frame: result of `preference_order()` on `df`, `response`, and `predictors`.

      + `f` (optional, function name): Function used to compute the association between the response and each predictor. If NULL (default), an appropriate function is selected by `f_default()`.
   + **Multicollinearity filtering**:
      + `cor_method` (character, optional): pairwise correlation method.
      ...
      + `max_vif` (numeric, optional): Maximum Variance Inflation Factor of all predictors. Recommended values between 2.5 and 10. If NULL, the VIF-based filtering is disabled.
