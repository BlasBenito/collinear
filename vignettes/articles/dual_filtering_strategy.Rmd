...
The function [collinear_select()](reference/collinear_select.html) applies two complementary methods to assess different aspects of multicollinearity: 
...
By combining a pairwise method (correlation) and a multivariate one (VIF), [collinear_select()](reference/collinear_select.html) is able to capture multicollinearity that either method alone might miss. At the same time, it respects predictor rankings to protect relevant variables during filtering.
...
The correlation matrix is computed with [cor_matrix()](reference/cor_matrix.html) and ordered after `preference_order`.
...
The subset of `m` with all rows and columns of `selected` and `candidate` is used as input for [vif()](reference/vif.html).
...
selected
```
Let's see if [collinear_select()](reference/collinear_select.html) returns the same selection.
