...
Let's take a look at a more tangible case now. The code below calls `collinear()` on the dataset [`vi_smol`](reference/vi_smol.html), which contains a numeric response [`vi_numeric`](reference/vi_numeric.html) (values of a vegetation index) and a bunch of numeric predictors named in the vector [`vi_predictors_numeric`](reference/vi_predictors_numeric.html).
...
Notice how `swi_min` and `swi_range` are selected, but `swi_max` and `swi_mean` are removed because they are collinear with `swi_min`. As said above, all predictors not in the argument `preference_order` were ranked from lower to higher mutual collinearity.
