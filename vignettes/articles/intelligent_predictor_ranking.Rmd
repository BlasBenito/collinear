...
You can download the Rmarkdown notebook used to render this article [here](https://github.com/BlasBenito/collinear/blob/main/vignettes/articles/intelligent_predictor_ranking.Rmd). To download the file, use the button *Download raw file* on the upper-right hand of the `Code` panel.
...
Let's consider a hypothetical: The user has dataframe `x` with three variables `a`, `b` and `c`, and domain knowledge indicating that `a` and `b` are key and should be preserved when possible. Then, the user calls `collinear()` as follows:
...
Let's use some real data now. The code below calls `collinear()` on the dataset [`spatialData::vi_smol`](https://blasbenito.github.io/spatialData/reference/vi_smol.html), which contains a numeric response `vi_numeric` (values of a vegetation index) and a bunch of numeric predictors named in the vector `vi_predictors_numeric` (generated from [`spatialData::vi_predictors`](https://blasbenito.github.io/spatialData/reference/vi_predictors.html) in the setup).
...
Notice how `swi_min` and `swi_range` are selected, but `swi_max` and `swi_mean` are removed because they are collinear with `swi_min`. All predictors not in the argument `preference_order` were ranked from lower to higher mutual collinearity.

Notice that there's a linear formula in the `formulas` slot of the output.
...
We can use it to fit a quick exploratory model, and save it for later.
...
Let's go back to our little hypothetical with the dataframe `x`, and the three variables `a`, `b` and `c`. But this time we also have a response `y`, and a user with not as much domain knowledge.
...
This functionality is implemented in the function `preference_order()`, which can take advantage of a `future` parallelization backend to speed-up operations. 
...
If we compare the R-squared of the two models we have created so far, we can see that using `preference_order()` has improved model fit.
...
Once you know your way around these functions, you can choose the one you prefer for your case. For example, below we replace [`f_auto`](reference/f_auto.html) with [`f_numeric_gam`](reference/f_numeric_gam) to fit univariate GAM models.
...
A gentle reminder to finish this section: `collinear()` runs `preference_order()` internally when `preference_order = NULL` and the argument `f` receives a valid function. And like `preference_order()`, it can use cross-validation to assess the association between response and predictor in a more robust manner.
...
Now you know the different ways you can take advantage of `collinear()` depending on your goals!
