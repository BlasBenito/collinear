...
This article describes how `collinear` combines target encoding, automated predictor ranking, a unified correlation framework for numeric and categorical variables, adaptive correlation and VIF thresholds, and a dual filtering strategy to deliver state-of-the-art multicollinearity filtering.
...
The package `collinear` includes the example data frame `collinear::vi_smol`,  with `r nrow(vi_smol)` rows and `r ncol(vi_smol)` columns with numeric and categorical responses and predictors.

The response columns are different representations of the same underlying data, and, have descriptive names: `vi_numeric`, `vi_counts` (integers), `vi_binomial` (1s and 0s), `vi_categorical` (five categories), and `vi_factor` (factor version of the previous one).

Predictor names are grouped in character vectors: `collinear::vi_predictors_numeric` (numeric and integer predictors), `collinear::vi_predictors_categorical` (character and factor predictors), and `collinear::vi_predictors`, containing them all.

## The Function [collinear()]

This function is the entry point to the full package functionality. It aims to facilitate multicollinearity filtering for any combination of categorical and/or numeric responses and predictors.
...
| **Preference order** | `f_numeric_r2()`: R-squared between `response` and `predictors` | `f_categorical_rf()`: Cramer's V of `response` against univariate random forest predictions |
...
```{r}
collinear::identify_categorical_variables(
  df = df_vi_numeric,
  predictors = vi_predictors
)
```
...
Here `f_auto()` selects `f_categorical_rf()`, which computes Cramer's V between categorical responses and predictors. 
...
selected <- collinear::identify_numeric_variables(
  df = df,
  predictors = selected
)
