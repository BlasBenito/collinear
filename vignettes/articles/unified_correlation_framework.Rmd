...
The function `cor_df()` combines several methods to compute pairwise associations between variables of different types:
  ...
  - *Numeric vs. categorical*: Combines target encoding and absolute Pearson correlation.
...
Let's start by setting up the session and loading the example data.
...
This is the simplest and most reliable case `cor_df()` handles, as absolute pairwise Pearson correlations are computed with the very fast `stats::cor()`. 
...
To handle this situation, `cor_df()` first applies target encoding to the categorical predictor using the numeric variable as reference, then applies `stats::cor()` to compute the absolute Pearson correlation.
```{r}
...
Let's try to answer that question empirically with a small simulation. It generates integer vectors with different cardinality levels and compares them using both `stats::cor()` and `collinear::cor_cramer()`. 
```{r}
set.seed(1)
