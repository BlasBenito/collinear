...
predictors_high_collinearity <- c(
...
The vector `predictors_high_collinearity` names predictors with a high multicollinearity, as the stats below show.
...
cor_plot <- experiment_adaptive_thresholds |> 
  ...
  ggplot2::labs(
    title = "Input vs. Output Median Correlation",
...
preds_plot <- experiment_adaptive_thresholds |> 
  ...
  ggplot2::scale_color_viridis_c(option = "turbo") + 
  ggplot2::labs(
    title = "Input Predictors vs. Selected Predictors",
    x = "Input Predictors",
...
cor_plot | preds_plot
```
The simulation results shows that the adaptive threshold selection works well in most use cases. Only in one case out the resulting selection had a maximum VIF higher than 2.5, and only `r (sum(experiment_adaptive_thresholds$cor_median_diff <= 0)/nrow(experiment_adaptive_thresholds))/100`% of the instances (cases over the diagonal in the lower-left corner of the scatterplot) the system produced suboptimal results.


## Step By Step

The adaptive filtering of multicollinearity thresholds requires three steps:

1. Computation of the median correlation of the input predictors via [cor_stats()].

```{r}
cor_median <- collinear::cor_stats(
  df = vi_smol,
  predictors = predictors_high_collinearity,
  quiet = TRUE
) |> 
...
2. Selection of the maximum between 0.58 and the median correlation as `max_cor`.

```{r}
max_cor <- max(0.58, cor_median)

max_cor
```

3. The pre-trained GAM model [gam_cor_to_vif] predicts a suitable `max_vif` from the given `max_cor`. 
...
- Maintains minimum filtering standards (max_cor ≥ 0.58)
