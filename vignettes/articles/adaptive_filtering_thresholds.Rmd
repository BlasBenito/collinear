...
To assess the effectiveness of this adaptive threshold selection method I ran `collinear()` on 10.000 random subsets between 4 and 100 randomly selected columns of a 500 columns and 3000 rows dataset, and compared the input and output multicollinearity stats and number of predictors across runs (see  *Annex I*).
...
```{r}
# =============================================================================
# Simulation: Testing Adaptive Multicollinearity Thresholds
# =============================================================================
# This script evaluates the effectiveness of the sigmoid-based adaptive
# threshold selection in collinear(). It runs 10,000 iterations on random
# subsets of a synthetic dataset with varying correlation structures,
# comparing input and output multicollinearity statistics.
# =============================================================================
...
# Generate correlated time series data using distantia
# The zoo_simulate() function creates realistic correlation structures
...
# Replicate to create a larger pool of predictors (500 columns)
# This ensures sufficient redundancy for multicollinearity testing
...
# Pre-allocate dataframe to store simulation results
iterations_df <- data.frame(
 input_predictors = sample(
   x = seq(from = min_cols, to = max_cols),
   size = n,
   replace = TRUE
 ),
 output_predictors = NA_integer_,
 input_cor_q75 = NA_real_,
 output_cor_q75 = NA_real_,
 input_cor_max = NA_real_,
 output_cor_max = NA_real_,
 input_vif_max = NA_real_,
 output_vif_max = NA_real_
)
...
progressr::with_progress({
 ...
 experiment_list <- future.apply::future_lapply(
   ...
   FUN = function(i) {
     ...
     # Compute input multicollinearity statistics
     ...
     iterations.df.i$input_cor_q75 <- input_stats |>
       dplyr::filter(method == "correlation", statistic == "quantile_0.75") |>
       dplyr::pull(value)

     iterations.df.i$input_cor_max <- input_stats |>
       dplyr::filter(method == "correlation", statistic == "maximum") |>
       dplyr::pull(value)

     iterations.df.i$input_vif_max <- input_stats |>
       dplyr::filter(method == "vif", statistic == "maximum") |>
       dplyr::pull(value)

     # Skip datasets with negligible multicollinearity
     # These don't test the adaptive threshold system meaningfully
     if (iterations.df.i$input_vif_max <= 2.5) {
       return(NULL)
     }

     # Apply adaptive multicollinearity filtering
     ...
     # Compute output multicollinearity statistics
     ...
     # Flag unexpected results for debugging
...
# Left panel: Input correlation vs output VIF
# Shows how the adaptive system bounds output VIF across correlation structures
cor_plot <- experiment_adaptive_thresholds |>
 ggplot2::ggplot() +
 ggplot2::aes(
   x = input_cor_q75,
...
# Right panel: Input vs output predictor counts
# Shows filtering intensity across different input sizes
preds_plot <- experiment_adaptive_thresholds |>
 ggplot2::ggplot() +
 ggplot2::aes(
   x = input_predictors,
...
# Reset parallelization backend
future::plan(sequential)

# Save results to package data
