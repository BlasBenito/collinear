...
In a linear model, the confidence interval of a predictor's estimate is widened by a factor equal to the square root of its Variance Inflation Factor (VIF). [This article](https://www.blasbenito.com/post/variance-inflation-factor/) goes deep into this topic, but the key detail here is that VIF, as a metric of the modelling uncertainty induced by multicollinearity, can also be used as a guideline to manage multicollinearity. 

The function `vif_select()` incorporates this idea onto an automated selection algorithm incorporating preference order to preserve important predictors. However, unlike `cor_select()`, `vif_select()` cannot work with categorical predictors unless these target-encoded.

The actual VIF computation is implemented in the function `vif_df()`.
